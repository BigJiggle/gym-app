...
export function registerPlanHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle('plan:generateDiet', async (_event, userId: number) => {
    ...
    const result = db
      ...
      .run([userId, planName, plan.calories_target, plan.protein_g, plan.carbs_g, plan.fat_g, plan.meal_count, JSON.stringify(plan.meals), plan.phase, dietWeeksOut ?? null, 2])
...
Ensure every exercise respects the constraints above while maintaining phase-appropriate intensity.`
    ...
    const dietNeedsUpgrade = ((lastDiet?.engine_version as number) ?? 1) < 2
    ...
    if (weeksOutChanged || dietNeedsUpgrade) {
      ...
      db.prepare(`INSERT INTO diet_plans (user_id, name, calories_target, protein_g, carbs_g, fat_g, meal_count, meals, phase, generated_at_weeks_out, engine_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run([userId, `${newDiet.phase.charAt(0).toUpperCase() + newDiet.phase.slice(1)} Nutrition Plan`, newDiet.calories_target, newDiet.protein_g, newDiet.carbs_g, newDiet.fat_g, newDiet.meal_count, JSON.stringify(newDiet.meals), newDiet.phase, weeksOut ?? null, 2])
      dietUpdated = true
    }
