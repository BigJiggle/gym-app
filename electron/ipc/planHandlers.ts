...
export function registerPlanHandlers(ipcMain: IpcMain): void {
  ...
  ipcMain.handle('plan:generateDiet', async (_event, userId: number) => {
    ...
    const result = db
      ...
      .run([userId, planName, plan.calories_target, plan.protein_g, plan.carbs_g, plan.fat_g, plan.meal_count, JSON.stringify(plan.meals), plan.phase, dietWeeksOut ?? null, 3])
...
Ensure every exercise respects the constraints above while maintaining phase-appropriate intensity.`
    ...
    // 4. Check diet — recalculate macros if weeks_out changed by ≥1, show context changed, or plan was generated by old engine
    ...
    const dietNeedsUpgrade = ((lastDiet?.engine_version as number) ?? 1) < 3
    ...
    if (weeksOutChanged || dietNeedsUpgrade) {
  ...
  ipcMain.handle('plan:applyAIRequest', async (_event, userId: number, userRequest: string, domain: 'training' | 'diet' | 'general') => {
    ...
    if (result.regenerateTraining) {
      ...
      const planResult = db.prepare(
        `INSERT INTO training_plans (user_id, name, weeks_total, phase, generated_at_weeks_out, user_modified) VALUES (?, ?, ?, ?, ?, ?)`
      ).run([userId, planMeta!.name, planMeta!.weeks_total, planMeta!.phase, weeksOut ?? null, 0])
      const planId = planResult.lastInsertRowid
      ...
      if (currentDiet) {
        ...
        const totals = (meals as any[]).reduce(
          (acc, m) => ({ cal: acc.cal + (m.calories ?? 0), pro: acc.pro + (m.protein_g ?? 0),
                          carb: acc.carb + (m.carbs_g ?? 0), fat: acc.fat + (m.fat_g ?? 0) }),
          { cal: 0, pro: 0, carb: 0, fat: 0 }
        )
        db.prepare(
          'UPDATE diet_plans SET meals=?, calories_target=?, protein_g=?, carbs_g=?, fat_g=? WHERE user_id=? AND id=(SELECT id FROM diet_plans WHERE user_id=? ORDER BY id DESC LIMIT 1)'
        ).run([JSON.stringify(meals), totals.cal, totals.pro, totals.carb, totals.fat, userId, userId])
        const saved = db.prepare('SELECT * FROM diet_plans WHERE user_id=? ORDER BY id DESC LIMIT 1').get(userId) as Record<string,unknown>
